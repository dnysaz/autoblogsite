import Header from '@/components/Header';
import Link from 'next/link';
import { getArticles } from '@/lib/articles';

export default function About() {
  const allArticles = getArticles();

  return (
    <div>
      <Header />

      <main className="main-grid">
        <article className="reader-container" style={{ margin: '0', maxWidth: '100%' }}>
          <Link href="/" className="nav-link" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem', fontSize: '0.9rem' }}>
            ← Home
          </Link>
          <h1 className="article-title">About AutoBlogSite.com</h1>
          
          <div className="info-page-body">
            <p>Welcome to <strong>AutoBlogSite.com</strong>, a living experiment in the future of content creation and AI automation.</p>
            
            <h2>What is AutoBlogSite.com?</h2>
            <p>Every article, insight, and meta description you see here is generated 100% by Artificial Intelligence (powered by AI Writer) without any human editorial intervention.</p>
            
            <h2>The Mission</h2>
            <p>Our mission is to explore the boundaries of what AI can achieve in the world of media. We believe that automation, when paired with powerful models like AI Writer, can produce high-quality, relevant, and engaging content at a scale never seen before.</p>

            <h2>How it Works</h2>
            <p>Our system uses a custom-built scheduler that triggers the AI Writer engine every few hours. The AI analyzes current trends, picks a relevant category, and writes an original, SEO-optimized article complete with Unsplash imagery and internal links.</p>
          </div>
        </article>

        <aside style={{ padding: '0 1rem', borderLeft: '1px solid #f2f2f2', height: 'fit-content', position: 'sticky', top: '100px' }}>
          <h3 style={{ fontWeight: 700, marginBottom: '1.5rem', fontSize: '1rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Recent Picks</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {allArticles.slice(0, 5).map(a => (
              <div key={a.slug}>
                <Link href={`/blog/${a.slug}`} className="sidebar-link">{a.title}</Link>
                <div style={{ fontSize: '0.75rem', color: '#6b6b6b', marginTop: '0.25rem' }}>DeepSeek AI in {a.category}</div>
              </div>
            ))}
          </div>

          <div style={{ marginTop: '4rem', padding: '1.5rem', background: '#f9f9f9', borderRadius: '8px', border: '1px solid #eee' }}>
            <h4 style={{ marginBottom: '0.5rem', fontSize: '0.9rem' }}>🤖 AutoBlogSite.com</h4>
            <p style={{ fontSize: '0.8rem', color: '#6b6b6b', lineHeight: 1.5 }}>
              100% AI generated content. No human intervention.
            </p>
          </div>
        </aside>
      </main>

      <footer style={{ background: '#f9f9f9', marginTop: '5rem', border: 'none' }}>
        <main style={{ maxWidth: '1040px', padding: '4rem 1.5rem' }}>
           <p style={{ fontSize: '0.9rem', color: '#6b6b6b' }}>&copy; {new Date().getFullYear()} AutoBlogSite.com AI Experiment.</p>
        </main>
      </footer>
    </div>
  );
}
